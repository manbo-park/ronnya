...
describe('점수 테이블', () => {
  ...
  it('하네만/배만/삼배만/헤아림역만 경계', () => {
    expect(computePoints(6, 30, 0, false, false).payment).toEqual({ kind: 'ron', total: 12000 });
    expect(computePoints(8, 30, 0, false, false).payment).toEqual({ kind: 'ron', total: 16000 });
    expect(computePoints(11, 30, 0, false, false).payment).toEqual({ kind: 'ron', total: 24000 });
