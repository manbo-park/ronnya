...
export function scoreHand(p: Problem): ScoringResult {
  ...
  const meldSets: SetInfo[] = p.melds.map((m) => {
    ...
    if (m.type === 'pon') return { kind: 'set', tile: id, open: true, quad: false, concealed: false };
    if (m.type === 'minkan') return { kind: 'set', tile: id, open: true, quad: true, concealed: false };
  ...
  for (const d of decomposeStandard(counts)) {
    ...
    d.groups.forEach((g, i) => {
      ...
      if (g.kind === 'run' && w >= g.tile && w <= g.tile + 2 && Math.floor(w / 9) === Math.floor(g.tile / 9)) {
        const pos = w - g.tile;
        let wait: WaitType = 'ryanmen';
        if (pos === 1) wait = 'kanchan';
        else if (pos === 2 && g.tile % 9 === 0) wait = 'penchan'; // 12 대기 3
  ...
  for (const it of interps) {
    ...
    if (
      ...
      (pts.totalReceived === best.pts.totalReceived && ev.han === best.ev.han && ev.fu > best.ev.fu)
    ) {
      best = { ev, pts };
    }
  }
...
function evaluate(
  ...
  if (it.pattern === 'chiitoi') {
    ...
    if (allTerminalOrHonor) {
      ...
      const groupsOk =
        sets.every((s) =>
          s.kind === 'run' ? s.tile % 9 === 0 || s.tile % 9 === 6 : isTerminalOrHonorId(s.tile),
  ...
  // 혼일색/청일색
  {
    const suitsUsed = new Set(
      allTiles.map(tileId).filter((id) => id < 27).map((id) => Math.floor(id / 9)),
    );
...
function computeFu(it: Interp, p: Problem, menzen: boolean): { fu: number; fuDetails: FuItem[] } {
  ...
  const pinfu =
    ...
    !(it.pairId >= 31 || it.pairId === 27 + (p.roundWind - 1) || it.pairId === 27 + (p.seatWind - 1)) &&
    it.wait === 'ryanmen';
